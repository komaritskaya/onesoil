...
let postCssPlugins = [
  ...
  mqpacker({
    // объединение медиавыражений с последующей их сортировкой
    sort: true
  }),
...
gulp.task('style', function() {
  return gulp
    .src(dirs.source + '/scss/style.scss') // какой файл компилировать
    .pipe(
      plumber({
        // при ошибках не останавливаем автоматику сборки
...
gulp.task('pug', function() {
  return gulp
    .src([dirs.source + '/*.pug', '!' + dirs.source + '/mixins.pug'])
    .pipe(plumber())
    .pipe(pug())
    .pipe(htmlbeautify())
    .pipe(gulp.dest(dirs.build));
...
gulp.task('copy:fonts', function() {
  return gulp
    .src([dirs.source + '/fonts/*.{ttf,woff,woff2,eot,svg}'])
...
gulp.task('img:opt', function(callback) {
  if (folder) {
    ...
    console.log(
      'Не указана папка с картинками. Пример вызова команды: folder=src/blocks/test-block/img npm start img:opt'
    );
    callback();
  }
});
...
gulp.task('sprite:png', function() {
  ...
  let cssStream = spriteData.css.pipe(gulp.dest(dirs.source + '/blocks/sprite-png/'));
